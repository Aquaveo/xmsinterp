...
public:
  ...
  virtual void TraverseLineSegment(double x1,
                                   ...
                                   VecInt& polyids,
                                   VecDbl& tvalues);
  virtual void TraverseLineSegment(double x1, double y1, double x2, double y2, VecInt& polyids);
...
private:
  ...
  void TraverseLineSegmentAll(double a_x1,
                              double a_y1,
                              double a_x2,
                              double a_y2,
                              VecInt& a_polyids,
...
void GmMultiPolyIntersectorImpl::BuildRTree()
{
  ...
  // GmMultiPolyIntersector2IntermediateTests::testLargeNumPolysAndSegments it didn't make much
  // difference, and actually may be very slightly slower, but the test may
  // not be very representative of real scenarios. The old code is
...
void GmMultiPolyIntersectorImpl::IntersectEachPolyWithLine()
{
  ...
  // m_rtree.query(bgi::overlaps(m_line), std::back_inserter(result)); // doesn't compile
  // m_rtree.query(bgi::covered_by(m_line), std::back_inserter(result)); // doesn't compile
  // m_rtree.query(bgi::covers(m_line), std::back_inserter(result)); // doesn't compile
...
///                  poly id -1.
//------------------------------------------------------------------------------
void GmMultiPolyIntersectorImpl::TraverseLineSegment(double x1,
                                                     double y1,
                                                     double x2,
                                                     double y2,
                                                     VecInt& polyids,
...
/// \param a_pts: Intersection points.
//-----------------------------------------------------------------------------
void GmMultiPolyIntersectorImpl::TraverseLineSegmentAll(double a_x1,
                                                        double a_y1,
                                                        double a_x2,
                                                        double a_y2,
                                                        VecInt& a_polyids,
              ...
              const VecDbl& a_expectedtValues)
{
  ...
  VecInt polyIds;
  VecDbl tValues;
  mpi->TraverseLineSegment(x1, y1, x2, y2, polyIds, tValues);
  TS_ASSERT_EQUALS_VEC(a_expectedPolyIDs, polyIds);
...
void GmMultiPolyIntersectorUnitTests::test1OutIn()
{
  ...
  iRunTest(-1, 5, 8, 5, pts, polys, expectedIds, expectedTvals);
...
void GmMultiPolyIntersectorUnitTests::test1InOut()
{
  ...
  iRunTest(5, 5, 11, 5, pts, polys, expectedIds, expectedTvals);
...
void GmMultiPolyIntersectorUnitTests::test1InIn()
{
  ...
  iRunTest(2, 5, 8, 5, pts, polys, expectedIds, expectedTvals);
...
void GmMultiPolyIntersectorUnitTests::test1OnOn()
{
  ...
  VecInt expectedIds{1, -1};
  VecDbl expectedTvals{0.0, 1.0};
  iRunTest(0, 5, 10, 5, pts, polys, expectedIds, expectedTvals);
...
void GmMultiPolyIntersectorUnitTests::test1OnIn()
{
  ...
  VecInt expectedIds{1, -1};
  VecDbl expectedTvals{0.0, 1.0};
  iRunTest(0, 5, 5, 5, pts, polys, expectedIds, expectedTvals);
...
void GmMultiPolyIntersectorUnitTests::test1InOn()
{
  ...
  VecInt expectedIds{1, -1};
  VecDbl expectedTvals{0.0, 1.0};
  iRunTest(5, 5, 10, 5, pts, polys, expectedIds, expectedTvals);
...
void GmMultiPolyIntersectorUnitTests::test1OutOn()
{
  ...
  iRunTest(-1, 5, 0, 5, pts, polys, expectedIds, expectedTvals);
...
void GmMultiPolyIntersectorUnitTests::test1EdgeOutOut()
{
  ...
  // iRunTest(-1, 10, 11, 10, pts, polys, expectedIds, expectedTvals); // doesn't work
...
void GmMultiPolyIntersectorUnitTests::test1EdgeOutIn()
{
  ...
  // iRunTest(-1, 0, 5, 0, pts, polys, expectedIds, expectedTvals); //works
  // iRunTest(-1, 9.999999999999999, 5, 10, pts, polys, expectedIds, expectedTvals); //works
...
void GmMultiPolyIntersectorUnitTests::test1EdgeInOut()
{
  ...
  VecInt expectedIds{1, -1};
  VecDbl expectedTvals{0.0, 0.833333};
  iRunTest(5, 10, 11, 10, pts, polys, expectedIds, expectedTvals); // works
  // iRunTest(11, 10, 5, 10, pts, polys, expectedIds, expectedTvals); // doesn't work
...
void GmMultiPolyIntersectorUnitTests::test2InOut()
{
  ...
  iRunTest(5, 5, 21, 5, pts, polys, expectedIds, expectedTvals);
...
void GmMultiPolyIntersectorUnitTests::test2InIn()
{
  ...
  iRunTest(5, 5, 15, 5, pts, polys, expectedIds, expectedTvals);
...
void GmMultiPolyIntersectorUnitTests::test2InEdgePt()
{
  ...
  // GmMultiPolyIntersectorImpl::GmMultiPolyIntersectorImpl to see that we don't set
  // z's to 0.0 and this test works. We are using Pt3d as a 2D boost geometry
  // so z's don't come into play.
...
void GmMultiPolyIntersectorUnitTests::testMap2MfBug()
{
  ...
  iRunTest(2046.1951363232131, 11842.783920653745, 1827.8055464926651, 11817.091027732373, pts,
           polys, expectedIds, expectedTvals);
  expectedIds = {1, 2, -1, 2, -1};
  expectedTvals = {0.0, .5, 1 - t, 1, 1};
  iRunTest(1827.8055464926651, 11817.091027732373, 2046.1951363232131, 11842.783920653745, pts,
           polys, expectedIds, expectedTvals);
} // GmMultiPolyIntersectorUnitTests::testInsideToEdgeThenThroughAdjacent
//------------------------------------------------------------------------------
/// \brief Given 1 x 2 2D grid turned into triangles with point at poly center
...
void GmMultiPolyIntersector2IntermediateTests::testLargeNumPolysAndSegments()
{
  ...
  VecInt polyIds;
  VecDbl tValues;
  const int nIntersectingLines = /*1e5*/ 1000; // Go big for serious timing testing
